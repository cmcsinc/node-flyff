...
| 2026-07-21 | implementor | resources/loaders/characterInc.loader.ts | **MMI enum lives in `game/resource/defineNeuz.h:92-314`** (NOT Mover.h/MsgHdr.h/defineWorld.h — those only reference it). MMI_DIALOG=0 (line 92), MMI_QUEST=1, MMI_TRADE=2, MAX_MOVER_MENU=175 (line 314). File is UTF-16LE with BOM + binary garbage (Korean) — strip BOM before utf16le decode. character.inc block format: `<Key>\n{\n…\n}` (not `<Key> {` same-line) — the regex `/^([A-Z][A-Za-z0-9_]*)\s*\{/gm` matches across the newline because `\s*` consumes `\n`. Commented-out `// SetEquip(...)` lines DO match the naive regex (// isn't respected by `[^)]`) — must strip `//`-to-EOL before tokenizing. II_* in SetEquip must be resolved via defineItem.h (items.yml schema doesn't keep the II_* key). |
...
| 2026-07-21 | implementor | systems/journalReplayers.ts + services/{combat,revival,inventory,questRewards}.service.ts | **Real WAL crash recovery shipped.** Key invariant: all DB writes are absolute-state (`updateLevelAndExp(level,exp)`, `updateGold(gold)`, `setItem(slot,itemId,count)`), so journal payloads MUST record absolute end-state, never deltas — replay is then idempotent (N rows in id order → final = last row; re-apply = no-op, no dupe/rollback). Canonical types `CHAR_EXP`/`CHAR_GOLD`/`INVENTORY_SLOT` replace the old delta types (EXP_GAIN/EXP_LOSS/EXP_CHANGE/GOLD_GAIN/GOLD_CHANGE/ITEM_PICKUP/SCROLL_CONSUME). Exp stored as JSON string for BigInt precision. Quest item/quest-state mutations NOT journaled (QuestService has no inventory sink — PERMISSIVE_INV stub; quest state is already write-through persisted by questRepo). Also: there is NO 30s `_dirty` flush loop — `_dirty` is vestigial; persistence is write-through only, so questRewards.grantExp previously had NO persist at all (WAL is now its only recovery). |
...
|-----------|------|----|---------|
| 2026-07-21 | implementor | all | **character.inc parser shipped (pending approval).** New `packages/resources/src/loaders/characterInc.loader.ts` parses raw/character.inc (UTF-16LE, BOM-stripped, `//` comments stripped) → per-NPC `{ menus: number[], hasDialog, outfit, dialogFile, vendorSlotCount }`. MMI enum source: `game/resource/defineNeuz.h:92-314` (MMI_DIALOG=0, MAX_MOVER_MENU=175) — copied to raw/ so loader resolves all 175 defines. II_* in SetEquip resolved to propItem ids via defineItem.h. Wired into `loadAllResources(dataDir, rawDir=default <dataDir>/../raw)` as `resources.characterInc: CharacterIncIndex`. `CMover.m_abMoverMenu: readonly number[]` added; `SpawnManager` NPC loop calls `blockForMover(idx, def.key)` and threads `menus` + block-sourced outfit (overrides yml outfit when block present). `scriptDlg.service` untouched (keeps resolving prefix via `m_szKey`). Tests: 12 new loader tests + 1 spawn-manager test for `m_abMoverMenu`. Resources 26/26, world 360/360 green. dist rebuilt. Real data: **360 blocks, 324 with MMI_DIALOG**, 47 trade, 5 banking, 29 outfits, 48 vendors. Not marked done per 2026-07-21 rule — pending approval. |
| 2026-07-21 | main (drops) | all | **Drops Phases D + E shipped — inventory + pickup, full kill→drop→pickup→bag loop complete.** Phase D: `SNAPSHOTTYPE_CREATEITEM=0x0003` + `createItem.serializer.ts` (96B one-slot, reuses Phase B CItemBase 20B + CItemElem 55B body) + `CPlayer.m_Inventory` (73-slot) + `JoinService.loadInventory` + compose wires `inventoryRepo`. Phase E: `core` `PACKETTYPE_ACTMSG=0x00ff0001` (MsgHdr.h:112) + `MAX_GOLD` (`constants/limits.ts`); `InventoryService` (`addItem` first-empty-slot + `addGold` MAX_GOLD clamp, WAL-first `INVENTORY_SLOT`/`CHAR_GOLD`, fire-and-forget `setItem`/`updateGold`); `net/snapshot/pointParam.serializer.ts` `buildSetPointParam`+`DST_GOLD=10000` (defineAttribute.h:349); `services/drop.service.ts` `isGoldSeed` export; `handlers/actMsg.handler.ts` (3×DWORD via **readDword** — objids ≥0x80000000 read negative signed; OBJMSG_PICKUP=11 → ownership → gold: addGold+SETPOINTPARAM self / item: addItem→CREATEITEM self → itemManager.remove broadcasts DEL_OBJ). Wired in compose+clientServer+index.ts. Gold-notify path verified at source: `CMover::AddGold` (Mover.cpp:614) → `AddSetPointParam(this,DST_GOLD,total)` (User.cpp:4624). 15 new tests (6 actMsg + 4 inventory + 5 createItem/drop-extra); **world 357/357 green**. ponytail: party loot-share + 7s FFA timeout, TID_GAME_LACKSPACE defined text on bag-full, item stacking (needs propItem nMaxStack), other OBJMSG_* (resurrect/collect). Plan: `C:\Users\Cyan\.claude\plans\wobbly-scribbling-fountain.md`. |
...
| 2026-07-21 | implementor | all | **GM COMMAND BATCH 2 SHIPPED — 15 new `/cmd` ports** (full C++ FuncTextCmd.cpp ON_TEXTCMDFUNC audit → portable-now subset). Self-mode toggles (MODIFYMODE broadcast, `constants/mode.ts` extended to mirror authorization.h:18-56 — added ONEKILL/NO_ATTACK/ITEM/COMMUNITY/OBSERVE/EXPUP_STOP): `/ok` `/nook` `/es` `/gmitem` `/gmnotitem` `/gmattck` `/gmnotattck` `/gmcommunity` `/gmnotcommunity` `/gmobserve` `/gmnotobserve`. New functional commands: `/out <name>` (GAMEMASTER2 disconnect target via `PlayerSocket.destroy?()` + `playerManager.remove`; optional `destroy?` added to PlayerSocket interface), `/ak` (radius-64m AoE kill via `spawnManager.inZone` + distSq filter + `spawnManager.kill` + new `buildRemoveObjs` batched DEL_OBJ; no exp/drops, no equipped-weapon gate — ponytail route through CombatService if GM-exp credit wanted), `/ci <itemId> [count]` (ADMIN createItem — **first consumer of InventoryService**, now wired in compose.ts via `new InventoryService({ inventoryRepo, charRepo, journal })`; numeric id only, IK3_VIRTUAL/EGG validation ponytail; uses `CreateItemSnapshotSerializer.buildOne`), `/ul` (userList — TEXT notice of live player names), `/stat <str\|sta\|dex\|int\|all> <n>` (persist via `charRepo.updateStats` + dirty; no SETSTATE snapshot/recompute — ponytail). Tests: command.service **50/50** (+16 new), world **349/349** green, tsc clean. Memory updated: `v15-gm-command-port-plan` (DONE 20→35). Still BLOCKED: `/p` `/g` (party/guild), `/cjob` (no `characterRepo.updateJob`), `/freeze` `/mute` `/talk` `/notalk` (per-target mode pipeline), `/pet*` `/cg` `/pkparam` `/cn` `/lua` `/loscr` `/rec` `/ritem` `/pier` (subsystems). |
| 2026-07-22 | main (quest) | all | **Quest item drops wired** (ponytail #3 of quest engine — its blockers inventory+loot now shipped). `@flyff/resources` `quest.loader.ts`: new `QuestDrop` type (`QuestItem` + `questId`), `QuestIndex.drops: Map<MI, QuestDrop[]>`, re-exported. `world-server` `systems/questTracker.system.ts` `onKill` → new `rollQuestDrops`: for each `dropsFor(victimMI)` whose quest the killer has active, cap at `SetEndCondItem` need (`itemNeed`), skip if `countItem >= need`, roll `rng.int(3_000_000_000) < prob`, `InventoryService.addItem` → CREATEITEM (new slot) / UPDATE_ITEM (stack-merge). Deps: `inventoryService?`, `createItemSerializer?`, `rng?` (default xRandomRng). `compose.ts`: InventoryService moved before questTracker + `CreateItemSnapshotSerializer` injected. Quest prob scale = 3e9 (quest 1 `QuestItem(38,6001,1500000000,1)` = 50%). 6 new tests (roll pass/fail, need-cap, objective-met skip, inactive-quest skip, stack-merge notify); world **448/448**, resources 34/34, 8/8 build clean. NOT user-verified — keep working. ponytail: questService still uses PERMISSIVE_INV (its `InventoryOps` shape ≠ `addItem` — needs adapter to make item-quest begin/end conditions real); dialog `source` bodies (BeginQuest/EndQuest) still the big blocker for auto-begin/complete via NPC. |
| 2026-07-22 | main (quest) | all | **questService PERMISSIVE_INV → real inventory adapter** (ponytail #2 — item begin/end conditions + rewards now functional, not stubbed). New `services/questInventory.adapter.ts`: `bindQuestInventory(player, {inventoryService, createItemSerializer})` → `{ inventory: QuestInventory, frames }`. `QuestInventory` (count/emptySlots/add/remove) moved here from quest.service.ts (re-exported). count/emptySlots scan the live `m_Inventory`; add → `InventoryService.addItem` + capture CREATEITEM (new slot) / UPDATE_ITEM (stack-merge); remove → scan slots, `InventoryService.consume` each, capture UPDATE_ITEM (count 0 on empty). `quest.service.ts`: dropped `inv`/`sink` getters → `context(player)` binds the real bag unless the `inventory?` test override is set; beginQuest/endQuest spread captured frames after SETQUEST. `compose.ts`: questService gets `inventoryService` + shared `createItemSerializer` (now reused by tracker too). Removals journal via consume (ITEM_CONSUME) — closes the "no WAL for quest item rewards" gap. 3 new tests (begin→CREATEITEM, end→reward CREATEITEM, end→removal UPDATE_ITEM) discriminated by SNAPSHOT sub-type @ byte 14; world **464/464** green, world-server build clean. NOT user-verified. Remaining blocker: dialog `source` bodies (BeginQuest/EndQuest/SetQuestState) — `LaunchQuest()` no-arg, quest id in compiled WorldDialog.dll not data; needs DLL RE or hand-built NPC→quest map. |
