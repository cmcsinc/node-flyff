...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (action === 'create') {
    ...
    if (portClash) {
      return NextResponse.json(
        { error: `Port ${String(parsed.instance.port)} already used by ${portClash.id}` },
        { status: 409 },
      );
    }
  ...
  if (action === 'delete') {
    ...
    if (await isRunning(id))
      return NextResponse.json({ error: 'Stop the server first' }, { status: 409 });
  ...
  if (action === 'update') {
    ...
    const merged = {
      ...instances[idx],
      ...(patch as Partial<ServerInstance>),
      id,
      type: instances[idx].type,
    };
