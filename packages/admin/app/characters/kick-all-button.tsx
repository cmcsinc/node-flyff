 ...
 */
export function KickAllButton({ onlineCount }: KickAllButtonProps) {
  ...
  return (
    <>
      ...
      {open && <KickAllModal onlineCount={onlineCount} onClose={() => setOpen(false)} />}
    </>
  );
}

function KickAllModal({ onlineCount, onClose }: KickAllButtonProps & { onClose: () => void }) {
  ...
  return (
    <Modal
      ...
      description={`${onlineCount} character${onlineCount === 1 ? "" : "s"} online`}
      className="max-w-md"
      footer={
        <>
    ...
    >
      <div className="space-y-4">
        ...
        <Field
          ...
          error={tooLong ? `Keep it under ${MAX_REASON_LEN} characters.` : undefined}
        >
          <Input
            id="kick-all-reason"
            value={reason}
