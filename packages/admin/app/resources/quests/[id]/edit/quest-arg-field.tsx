...
function argLabel(spec: QuestArgSpec | undefined, index: number, group?: number): string {
  const base = spec ? spec.name.replace(/([a-z])([A-Z])/g, "$1 $2") : `Argument ${String(index + 1)}`;
...
export function QuestArgField({
  ...
  const options =
    INLINE_ENUMS[kind] ??
    (registry !== undefined ? (ctxOpts[registry] as typeof INLINE_ENUMS[QuestArgKind]) : undefined);
    ...
    return (
      <div className="flex items-start gap-3 py-1">
        <Switch
          ...
          onChange={(e) => { emit(e.target.checked ? "1" : "0"); }}
        />
