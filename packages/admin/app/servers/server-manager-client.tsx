...
export function ServerManager({ initial }: { initial: InstanceStatus[] }): React.JSX.Element {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (
            ...
            setInstances(json.instances as InstanceStatus[]);
          }
        }
      } catch {
  ...
  const act = async (action: 'start' | 'stop' | 'delete', id: string): Promise<void> => {
    setBusy(id);
    try {
  ...
  const setAutoStart = async (id: string, enabled: boolean): Promise<void> => {
    setBusy(id);
    try {
          ...
          {instances.length === 0 ? (
            ...
            instances.map((inst) => (
              <InstanceRow
                ...
                onAction={(a) => {
                  if (a === 'delete') { setPendingDelete(inst); }
                  else { void act(a, inst.id); }
                }}
                onAutoStart={(instId, enabled) => {
                  void setAutoStart(instId, enabled);
                }}
              />
            ))
          )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {active && (
            <p className="font-mono text-xs text-muted-foreground">
              ...
              {active.pid !== null && active.state === 'running'
  ...
  return (
    ...
    >
      ...
      >
        <span
          aria-hidden="true"
          className={cn('h-2 w-2 shrink-0 rounded-full', STATE_DOT[inst.state])}
        />
        <span className="min-w-0 flex-1">
          ...
          <span className="block truncate font-mono text-[11px] text-muted-foreground">
            ...
            {inst.state === 'exited' ? ` · code ${String(inst.exitCode)}` : ''}
          </span>
        </span>
      ...
      <div className="mt-2 flex items-center gap-1.5">
        ...
        >
          <Switch
            ...
            disabled={busy}
            onChange={(e) => {
              onAutoStart(inst.id, e.target.checked);
            }}
          />
          auto
        </label>
...
function CreateInstanceModal({
  ...
  const submit = async (e: React.SyntheticEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    setSaving(true);
    try {
  ...
  return (
    ...
    >
      <form id={formId} className="grid gap-x-6 gap-y-5 sm:grid-cols-2" onSubmit={(e) => { void submit(e); }}>
        <div className="space-y-1.5">
          ...
          <Select
            ...
            value={type}
            onChange={(e) => {
              setType(e.target.value as ServerType);
            }}
          >
        ...
        {type === 'world' && (
          <p className="rounded-lg border border-warning/30 bg-warning/10 p-3 text-xs text-foreground sm:col-span-2">
            A new world also needs a unique{' '}
            <code className="font-mono">registration.channelId</code> in its overrides, and its id
            added to the cluster&apos;s{' '}
            <code className="font-mono">registration.allowedWorlds</code> — otherwise the cluster
            rejects it.
          </p>
        )}
      </form>
