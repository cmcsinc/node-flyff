...
describe("CreateAccountSchema", () => {
  ...
  it("maps an empty email to null, not an empty string", () => {
    assert.equal(CreateAccountSchema.parse({ username: "abcd", password: "test", email: "" }).email, null);
  });
  ...
  it("rejects usernames the certifier would reject", () => {
    ...
    for (const username of ["ok_4", "Mixed_Case_9", "a".repeat(32)]) {
      assert.equal(CreateAccountSchema.safeParse({ username, password: "test" }).success, true, username);
    }
  });

  it("rejects short and over-long passwords", () => {
    assert.equal(CreateAccountSchema.safeParse({ username: "abcd", password: "abc" }).success, false);
    assert.equal(CreateAccountSchema.safeParse({ username: "abcd", password: "a".repeat(21) }).success, false);
  });
});
...
describe("UpdateAccountSchema", () => {
  ...
  it("maps an empty bannedUntil to null and accepts an ISO stamp", () => {
    ...
    assert.equal(UpdateAccountSchema.safeParse({ id: 1, bannedUntil: "2030-01-02" }).success, false);
  });
});
...
describe("fieldErrors", () => {
  ...
  it("returns an empty object when valid", () => {
    assert.deepEqual(fieldErrors(CreateAccountSchema, { username: "tester", password: "test" }), {});
  });
});
