...
void describe('setArg', () => {
  void it('changes only the addressed arg', () => {
    ...
    assert.deepEqual(
      next.commands[1].args.map((a) => a.value),
      [100, 999],
    );
    assert.deepEqual(
      next.commands[2].args.map((a) => a.value),
      [75, 129],
    );
  });
  ...
  void it('pads intermediate slots so a filled optional arg lands in its own position', () => {
    ...
    assert.deepEqual(
      next.commands[0].args.map((a) => a.value),
      [0, 1, 10, 1500],
    );
  });
...
void describe('buildPutBody', () => {
  ...
  void it('sends the full command list when one arg changed', () => {
    ...
    assert.deepEqual(
      body.commands[2].args.map((a) => a.value),
      [80, 129],
    );
  });
  ...
  void it('drops removed commands from the emitted list', () => {
    ...
    assert.deepEqual(
      body.commands?.map((c) => c.cmd),
      ['SetRepeat', 'SetBeginCondLevel'],
    );
  });
