...
describe('itemRows', () => {
  ...
  it('sorts by id', () => {
    assert.deepEqual(
      [...rows].sort((a, b) => a.id - b.id).map((r) => r.id),
      rows.map((r) => r.id),
    );
  });
});
...
describe('skillRows', () => {
  it('carries the job marker from the source file onto every row', () => {
    ...
    assert.ok(rows.length > 100);
    assert.ok(
      rows.every((r) => r.job.length > 0),
      'every skill file declares a _job',
    );
...
describe('dropRows', () => {
  ...
  it('resolves the MI_ key to a monster name', () => {
    ...
    assert.ok(
      resolved > rows.length * 0.9,
      `only ${String(resolved)}/${String(rows.length)} names resolved`,
    );
  });
...
describe('dialogueRows', () => {
  it('skips index files and resolves NPC names for most dialogues', async () => {
    ...
    assert.ok(
      rows.every((r) => r.prefix.length > 0),
      'prefix-less index files must be skipped',
    );
    assert.equal(new Set(rows.map((r) => r.ref)).size, rows.length, 'row keys must be unique');
    assert.ok(
      rows.some((r) => r.npcName.length > 0),
      'expected some names to resolve',
    );
    assert.ok(
      rows.every((r) => r.inert <= r.states),
      'inert count cannot exceed state count',
    );
