...
describe("sortRows", () => {
  ...
  it("sorts numbers numerically, not lexically", () => {
    ...
    assert.deepEqual(out.map((r) => r.level), [2, 10, 10]);
  });

  it("sorts strings case-insensitively", () => {
    const out = sortRows(ROWS, { key: "name", dir: "asc" });
    assert.deepEqual(out.map((r) => r.name), ["Alpha", "beta", "gamma"]);
  });
  ...
  it("does not mutate the input array", () => {
    ...
    assert.deepEqual(rows.map((r) => r.id), [3, 1, 2]);
  });
});
...
describe("sortHref", () => {
  ...
  it("preserves other filters but drops page (a new order invalidates the offset)", () => {
    const href = sortHref({ search: "abc", page: "4", perPage: "50" }, "id", { key: "", dir: "asc" });
