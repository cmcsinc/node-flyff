...
export class ScriptDlgService {
  ...
  private async runState(
    ...
    if (keyIdx === 0) this.synthInitialMenu(player, npc, state, file, frames);
  ...
  private synthInitialMenu(
    player: CPlayer, npc: CMover, state: DialogState, file: DialogFile | undefined, frames: Buffer[],
  ): void {
    ...
    // Quest-offer buttons: the compiled `WorldDialog.dll` would have emitted
    // these from state 0's body (GetQuestState -> AddKey). With the dll absent
    // and most quest NPCs' state-0 source holding only `Speak(...)`, the
    // quest-give state is otherwise unreachable. Synthesize one Accept button
    // per begin-eligible quest + one Complete button per active end-eligible quest.
    this.appendQuestButtons(player, npc, funcs);
    funcs.unshift({ type: 'removeAllKeys' });
    frames.push(this.scriptDialog.build(player.m_idPlayer, funcs));
  }

  /**
   * Append Accept/Complete buttons for quests bound to this NPC. Accept shows
   * for quests the player can begin (in `beginByKey`, not yet begun/complete);
   * Complete shows for active quests in `endByKey`. Routes use synthetic
   * `#b<id>` / `#e<id>` keys that {@link parseQuestRoute} resolves to begin/end.
   */
  private appendQuestButtons(player: CPlayer, npc: CMover, funcs: ScriptFunc[]): void {
    const lk = npcLookupKey(npc);
    if (!lk) return;
