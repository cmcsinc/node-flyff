...
export function buildWorldClientServer(deps: WorldClientServerDeps): {
  ...
  dispatcher.register(PACKETTYPE.SFX_ID, () => { /* no-op: SFX_ID is info-only; see comment above */ });
  dispatcher.register(PACKETTYPE.SFX_HIT, () => { /* no-op: SFX_HIT is info-only; see comment above */ });
