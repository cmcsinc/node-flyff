...
describe('writeItemContainer', () => {
  ...
  it('populated: occupied body = BYTE slot + 78 B CItemElem, then trailing adwObjIndex', () => {
    ...
    assert.equal(buf.readUInt32LE(92), NULL_ID, 'adwObjIndex[0] empty');
    assert.equal(buf.readUInt32LE(96), 1, 'adwObjIndex[1] = slot index');
    assert.equal(buf.readUInt32LE(100), NULL_ID, 'adwObjIndex[2] empty');
